# ScoRack Admin Platform — Phase 2a + 2b

Auth, the operator shell, and now the actual tenant lifecycle: create,
rename, change sport, and move a tenant between active, suspended, and
blocked.
...
- **Tenant detail** (`/tenants/:id`): edit the name, slug, and sport,
  and the status control that actually blocks a tenant, RLS enforces
  it the instant you click, no redeploy needed

Division setup, rosters, and bracket generation are still ahead.
...
If you already deployed Phase 2a, this is just a normal update: pull
the new files into the same repo (or re-upload over them) and push,
Vercel redeploys automatically. No new environment variables, no new
migration, `/tenants` works against the schema Phase 1 already set up.
